...
def add_dark_exp(tab, inst, log, raw_dir):
    ...
    data_dict = {'Object': objects, 'Start': starts, 'End': ends, 
                 'ExpTime': exptime, 'Coadds': np.full(len(objects), np.nan), 
                 'Expose': exposes, 'Total_tint': np.full(len(objects), np.nan), 
                 'Filter': np.full(len(objects), np.nan), 
                 '''Dither (")''': np.full(len(objects), np.nan), 
                 'Aperture': np.full(len(objects), np.nan), 'TUB': np.full(len(objects), np.nan), 
                 'Airmass': np.full(len(objects), np.nan), 'PT': np.full(len(objects), np.nan), 
