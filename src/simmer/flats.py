...
def flat_driver(raw_dir, reddir, config, inst, plot=True):
    ...
    for filter_name in tqdm(filts, desc='Running flats', position=0, leave=True):
        ...
        flatlist = eval(_flats[_flats.Filter == filter_name].Filenums.values[0]) # pylint: disable=eval-used 
        itime = _flats[_flats.Filter == filter_name].ExpTime.values[0]
        darkfile = reddir + f'dark_{int(round(itime))}sec.fits'
        create_flats(raw_dir, reddir, 
                     flatlist, darkfile, inst,
                     plot=plot, filter_name=filter_name)


def create_flats(raw_dir, reddir, flatlist, darkfile, inst, 
                 filter_name=None, test=False, plot=True):
    ...
    if plot:
        u.plot_array(flat_array, -2., 2., reddir, f'flat_cube_{filt}.png')
