...
class TestCreation(unittest.TestCase):
	...
	def test_create_flats(self):
		...
		result = flats.create_flats(raw_dir, reddir, flatlist, np.nan, self.inst, test=True)
	...
	def test_create_imstack(self):
		...
		s_dir = raw_dir
		result, shifts_all = image.create_imstack(raw_dir, reddir, s_dir, imlist, self.inst,)
		compare_list = ['compare_create_imstack_0', 'compare_create_imstack_1', 
						'compare_create_imstack_2', 'compare_create_imstack_3']
		compare_imstack = np.array([np.loadtxt(raw_dir + file) for file in compare_list])
	...
	def test_create_im_default(self):
		...
		raw_dir, reddir = 'src/simmer/tests/sky_test/', 'src/simmer/tests/sky_test/'
		s_dir = raw_dir
		compare_final_im = pyfits.getdata(raw_dir + 'Ks/compare_final_im_default.fits', 0)
...
class TestDrivers(unittest.TestCase):
	...
	def test_image_driver(self):
		...
		remove_files = ['sh00.fits', 'sh01.fits', 'sh02.fits', 'sh03.fits', 'shifts.txt']
...
class TestIntegration(unittest.TestCase):
	...
	def test_PHARO_all_drivers(self):
		...
		raw_dir, reddir = os.getcwd() + '/src/simmer/tests/PHARO_integration/', os.getcwd() + '/src/simmer/tests/PHARO_integration/'
	...
	def test_PHARO_config_drivers(self):
		...
		raw_dir, reddir = os.getcwd() + '/src/simmer/tests/PHARO_config_driver/', os.getcwd() + '/src/simmer/tests/PHARO_config_driver/'
		config_file = os.getcwd() + '/config.csv'
	...
	def test_PHARO_image_drivers(self):
		print('Testing PHARO image integration') # need to actually point to correct folder, change aodirs
		download_folder('PHARO_image_driver')
		raw_dir, reddir = os.getcwd() + '/src/simmer/tests/PHARO_image_driver/',  os.getcwd() + '/src/simmer/tests/PHARO_image_driver/'
		config_file = os.getcwd() + '/config.csv'
		drivers.image_driver(self.p, config_file, raw_dir, reddir)
		remove_files = ['sh00.fits', 'sh01.fits', 'sh02.fits', 'sh03.fits', 'shifts.txt']
		val = np.all([r in os.listdir(raw_dir + 'HIP49081/Br-gamma/') for r in remove_files])
