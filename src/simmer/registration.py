...
def roll_shift(image, shifts, cval=0.):
...
def register_bruteforce(image, rough_center=None):
    ...
    if rough_center:
        ...
        small_image = image[rough_center[0] - scale : rough_center[0] + scale,
                            rough_center[1] - scale : rough_center[1] + scale]
        # now do binary search
        small_coordinates = search_threshold(small_image)
        coordinates_y = [coord[0] + (rough_center[0] - scale) for coord in small_coordinates]
        coordinates_x = [coord[1] + (rough_center[1] - scale) for coord in small_coordinates]
...
def register_saturated(image, searchsize1, newshifts1, rough_center=None):
    ...
    if rough_center:
        scale = np.floor(im_shape[0] / 50)
        image = image[rough_center[0] - scale : rough_center[0] + scale,
                      rough_center[1] - scale : rough_center[1] + scale]
...
def rot_search(dat, x_initial, y_initial, xrad, yrad):
    ...
    x_grid, y_grid = np.meshgrid(np.arange(xoffset + xrad, xoffset - xrad - 1, -1), 
...
#         dat = list(map(int, d))
    ...
    if xshift > 0:
        ...
        dat2[:, dat.shape[1] + xshift:dat.shape[1]] = 0 #plus sign because xshift is negative
...
def calc_shifts(dat, x_initial, y_initial, xrad, yrad, find='max', method='radon'):
    ...
    x_grid, y_grid = np.meshgrid(np.arange(xoffset + xrad, xoffset - xrad - 1, -0.01), 
...
def run_rot(image, searchsize, center, newsize):
    ...
    cut_image = image[int(center[0] - newsize / 2):int(center[0] + newsize / 2), \
    ...
    (xshift, yshift), res = calc_shifts(cut_image, newcent[0], newcent[1], 
                                        searchsize, searchsize, find='min', 
                                        method='rotate')
