...
def dark_driver(raw_dir, reddir, config, inst, plot=True):
    ...
    for texp in tqdm(texps, desc='Running darks', position=0, leave=True):
        ...
        darklist = eval(_darks[_darks.ExpTime == texp].Filenums.values[0]) # pylint: disable=eval-used        
        create_darks(raw_dir, reddir, darklist, inst, plot=plot) # creates a new dark file
...
def create_darks(raw_dir, reddir, darklist, inst, plot=True):
    ...
    if plot:
        u.plot_array(dark_array, -1., 50., reddir, f'dark_cube_{int(round(itime))}sec.png')
