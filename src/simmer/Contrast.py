...
def first_cc_val_neg(param,*args):
    ...
    first_ap = CircularAnnulus([center_x,center_y],r_in=radius_size,r_out=2*radius_size)
...
def find_best_center(data,radius_size,starting_center=[299.5,299.5]):
    ...
    center = scipy.optimize.differential_evolution(first_cc_val_neg,((x_min,x_max),(y_min,y_max)),args=(data,radius_size))
    print('Best Center at',[center.x[0],center.x[1]],'with first delta mag of',-1*first_cc_val_neg((center.x[0],center.x[1]),data,radius_size))
    return(center.x[0],center.x[1],first_cc_val_neg((center.x[0],center.x[1]),data,radius_size))

def ConCur(star_data,radius_size=1,center=None,background_method='astropy',find_hots=False,find_center=False):
    ...
    all_apers, all_apers_areas, all_masks = [center_ap], [center_ap.area], [center_ap.to_mask(method='exact')]

    all_data, all_weights = [all_masks[0].multiply(data)], [all_masks[0].multiply(ones)]
    
    all_stds = [twoD_weighted_std(all_data[0],all_weights[0])]
    
    for j in range(int(number_of_a)):
        aper = CircularAnnulus([center[0],center[1]],r_in=(j*radius_size+radius_size),r_out=(j*radius_size + 2*radius_size))
    ...
    for i in range(len(delta_mags)):
        arc_lengths.append((i*0.033 + 0.033)*radius_size) #make sure center radius size is correct
    ...
    if delta_mags[1]<0:
        print('Warning: first annulus has negative relative flux of value,','%.5f' % delta_mags[1],'consider changing center or radius size')
