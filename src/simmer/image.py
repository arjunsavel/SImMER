...
def image_driver(raw_dir, reddir, config, inst, plot=True):
    ...
    if inst.take_skies:
        ...
        skies = config[(config.Object != 'flat') \
                       & (config.Object != 'dark') & (config.Object != 'setup')]
...
#             imlist = literal_eval(obj[obj.Comments != 'sky'].Filenums.values[n])
            imlist = eval(obj[obj.Comments != 'sky'].Filenums.values[n]) # pylint: disable=eval-used # liter_eval issues
            ...
            create_imstack(raw_dir, reddir, s_dir, 
                           imlist, inst, 
                           plot=plot, filter_name=filter_name)
    return methods
...
#     method = 'brute force' # the default
    ...
    if inst.name == 'PHARO' and filt == 'Br-gamma': #not sure whether this is generalizable
        flatfile = reddir + 'flat_K_short.fits'
    flat = pyfits.getdata(flatfile, 0)

    skyfile = sf_dir + 'sky.fits'
    sky = pyfits.getdata(skyfile, 0)
    sky[sky != sky] = 0. #set nans from flat=0 pixels to 0 in sky
...
#             saturateds.append(reg.test_saturated)
        ...
        hdu.writeto(sf_dir+'sh{:02d}.fits'.format(i), overwrite=True, output_verify='ignore')
    
    if nims > 50: 
        plot = False
        
    if plot:
        u.plot_array(im_array, -10., 10000., sf_dir, 'shift1_cube.png')
...
def create_im(s_dir, ssize1, plot=True, fdirs=None, method='default'):
    ...
    for sf_dir in fdirs: # each filter
        ...
        for i in range(nims):  #each image
            ...
            if method == 'saturated':
                ...
                if len(image_centered) == 0:
                    ...
                    image_centered, rot, newshifts1 = reg.register_saturated(image, 
                                                                             ssize1,
                                                                             newshifts1)
                    rots[i, :, :] = rot
            elif method == 'saturated wide':
                rough_center = reg.find_wide_binary(image)
                image_centered = reg.register_bruteforce(image, rough_center=rough_center)
            elif method == 'wide':
                rough_center = reg.find_wide_binary(image)
                image_centered = reg.register_bruteforce(image, rough_center=rough_center)
        ...
        if plot:    
            u.plot_array(rots, 0., 1., sf_dir, 'rots.png', 
                         extent=[-ssize1, ssize1, -ssize1, ssize1])
            
            fig = plt.figure(figsize=(10, 10))
            ax = fig.add_subplot(1, 1, 1) # pylint: disable=invalid-name # common axis name!
            ax.imshow(final_im, cmap='plasma', origin='lower', 
                      norm=co.Normalize(vmin=0., vmax=10000.))
            plt.savefig(sf_dir + 'final_image.png')
            plt.close('all')

                        
            nrows = 4
            ncols = int(np.ceil((nims / 4.)))
            rowheight = nrows * 10
            colheight = ncols * 10
            ...
            for i in range(nims):
                ...
                ax.imshow(np.rot90(frames[i, 250:350, 250:350], 2), cmap='plasma', 
                          norm=co.Normalize(vmin=0., vmax=10000.))
