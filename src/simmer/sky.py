    ...
    for star in tqdm(stars, desc='Running skies', position=0, leave=True):
        ...
        for n, filter_name in enumerate(filts): 
           ...
           # literal_eval issues below
            skylist = eval(skies[skies.Object == star].Filenums.values[n]) # pylint: disable=eval-used 
            #end CDD change
            create_skies(raw_dir, reddir, 
                         s_dir, skylist, inst, 
...
def create_skies(raw_dir, reddir, s_dir, skylist, inst, plot=True, filter_name=None):
    ...
    if plot:
        u.plot_array(sky_array, -10., 100., sf_dir, 'sky_cube.png')
    #CDD change   
