...
class ShARCS(Instrument):
    ...
    replace_filters = {'BrG-2.16': ['Ks', 'K'], 'K+CH4-2.4': ['Ks', 'K'], 'J+CH4-1.2':'J', \
                        'H2-2.2122': ['Ks', 'K']}
    ...
    def adjust_array(self, array, nims):
        return np.array([u.image_subsection(array[dd, :, :], 
                                            self.npix, self.center) 
    ...
    def bad_pix(self, image):
        ...
        image[bad] = filtered[bad] # replace bad pixels with median of surrounding pixels
...
class PHARO(Instrument):
    ...
    plate_scale = 0.025
    filter_logtohead = {'Ks':'K_short', 'BrG':'Br-gamma', 'BrG+H2':'Br-gamma', 'J':'J'}
    filter_headtolog = {'K-short':'Ks', 'Br-gamma':'BrG', 'J':'J'}
    replace_filters = {'BrG-2.16': ['Ks', 'K'], 'K+CH4-2.4': ['Ks', 'K'], \
                       'J+CH4-1.2':'J', 'H2-2.2122': ['Ks', 'K']}
    file_prefix = 'sph'
    plim = (462, 562)
    off = (-462, -462)
