...
outdir = (
    startname + datestr + midname + datestr + "/" + starname + "/" + filt + "/"
)
...
def analyze(
    ...
    while np.logical_and(
        niter < maxiter, np.logical_or(posdiff > postol, fwhmdiff > fwhmtol)
    ):
        ...
        updated_xcen, updated_ycen = find_center(
            updated_sources, verbose=verbose
        )
        ...
        # Compute differences
        posdiff = np.sqrt(
            (updated_xcen - xcen) ** 2.0 + (updated_ycen - ycen) ** 2.0
        )
        fwhmdiff = np.sqrt((updated_fwhm - fwhm) ** 2.0)
    ...
    contrast_curve = sim_con_curve.contrast_curve_main(
        im, fwhm, inst, position=[xcen, ycen]
    )
...
def find_sources(
    ...
    while np.logical_and(type(sources) == type(None), fwhm < 200):
        ...
        daofind = DAOStarFinder(
            fwhm=fwhm, threshold=tscale * std, exclude_border=True, **kwargs
        )
...
def find_FWHM(image, center, min_fwhm=2, verbose=False):
    ...
    closest_x = np.unravel_index(np.argsort(diff.flatten()), image.shape)[0][
        :10
    ]  # 10 pixels closest in value to HM
    closest_y = np.unravel_index(np.argsort(diff.flatten()), image.shape)[1][
        :10
    ]
...
def aperture_photometry(im, df, fwhm):
    ...
    annulus_apertures = CircularAnnulus(
        positions, r_in=fwhm * 2, r_out=fwhm * 3
    )
